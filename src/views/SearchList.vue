<template>
  ...
  <div class="search-list mtop-20">
    ...
    <TabMenu :menuList="text" @menuClick="handMenuClick" />
    <el-skeleton v-if="isLoading" :rows="12" animated />
    <template v-else-if="total !== 0">
      <!-- 歌曲 -->
      <MusicList
        v-if="vIndex === '0'"
        :tableData="searchList.musicList"
      ></MusicList>
      <!-- 专辑 -->
      <InfoList
...
<script>
import { search } from "@/api/api-other";
...
export default {
  ...
  methods: {
    ...
    toArtistDetail(row) {
      this.$router.push("/singerdetail/" + row.id);
    },
    toPlayListDetail(row) {
      this.$router.push("/songlistdetail/" + row.id);
    },
    toUserDetail(row) {
